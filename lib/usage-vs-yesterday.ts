export function formatVsYesterdayCount(
  current: number,
  baseline: number
): string | null {
  ...
  if (baseline === 0) {
    return "↑ vs y'day same time";
  }
  const pct = Math.round(((current - baseline) / baseline) * 100);
  if (pct === 0) {
    return "Flat vs y'day same time";
  }
  const sign = pct > 0 ? "+" : "";
  return `${sign}${pct}% vs y'day same time`;
}

export function formatVsYesterdayLatency(
  currentMs: number,
  baselineMs: number
): string | null {
  ...
  if (currentMs === 0) {
    return "-100% avg vs y'day same time";
  }
...
export function buildVsYesterdayStrings(
  ...
  return {
    ...
    avgApiDurationMs: formatVsYesterdayLatency(
      today.avgApiDurationMs,
      yesterday.avgApiDurationMs
    ),
    interactions: formatVsYesterdayCount(
      today.interactions,
      yesterday.interactions
    ),
