...
export async function listBriefs(): Promise<CategoryBrief[]> {
  ...
  if (error) {
    throw new Error(`listBriefs failed: ${error.message}`);
  }
  ...
  return [...latestByCategory.values()].map(rowToBrief);
}

export async function getBriefByCategory(
  slug: CategorySlug
  ...
  if (error) {
    throw new Error(`getBriefByCategory failed: ${error.message}`);
  }
...
export async function upsertBrief(brief: CategoryBrief): Promise<void> {
  ...
  if (error) {
    throw new Error(`upsertBrief failed: ${error.message}`);
  }
}
