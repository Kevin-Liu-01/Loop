...
function brand(key: keyof typeof BRAND_LOGOS, alt: string): IconRef {
  return { alt, kind: "url", url: BRAND_LOGOS[key] };
}
...
export interface PlatformDocIcon {
  src: string;
  alt: string;
}

export const PLATFORM_DOC_ICONS: Record<string, PlatformDocIcon> = {
  agents: { alt: "GitHub / AGENTS.md", src: `${SI}/github` },
  claude: { alt: "Anthropic / Claude", src: "/brands/anthropic.svg" },
  codex: { alt: "OpenAI / Codex", src: "/brands/openai.svg" },
  cursor: { alt: "Cursor", src: "/brands/cursor.svg" },
};
...
export function getMcpIcon(name: string, homepageUrl?: string): IconRef {
  ...
  if (homepageUrl) {
    return { alt: name, kind: "url", url: computeSourceLogoUrl(homepageUrl) };
  }
  return lucide("plug");
}
