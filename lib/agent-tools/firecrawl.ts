import { FIRECRAWL_API_URL, FIRECRAWL_TIMEOUT_MS } from "@/lib/agent-tools/constants";
...
async function firecrawlRequest<T>(endpoint: string, body: Record<string, unknown>): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Firecrawl ${endpoint} returned HTTP ${response.status}: ${text.slice(0, 200)}`);
  }

  return response.json() as Promise<T>;
}

export async function firecrawlSearch(query: string, limit: number): Promise<FirecrawlSearchResponse> {
  return firecrawlRequest<FirecrawlSearchResponse>("/search", { query, limit });
}
