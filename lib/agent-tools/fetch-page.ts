...
async function fetchWithBrowser(
  ...
  try {
    ...
    await runBrowserCommand(["wait", "--load", "networkidle"], {
      sessionId,
      timeoutMs: BROWSER_COMMAND_TIMEOUT_MS,
    });

    const title = await runBrowserCommand(["get", "title"], {
      sessionId,
      timeoutMs: 5_000,
    });

    const snapshot = await runBrowserCommand(
      ["snapshot", "-c", "-d", String(BROWSER_SNAPSHOT_DEPTH)],
      { sessionId, timeoutMs: 10_000 }
    );

    const content = snapshot.slice(0, maxChars);
    return {
      url,
...
export async function fetchPageContent(
  ...
  try {
    ...
    if (browserReady) {
      try {
        return await fetchWithBrowser(url, maxChars);
      } catch (browserError) {
        console.warn(
          `[fetch_page] Browser fetch failed, falling back to HTTP: ${
...
export function buildFetchPageTool() {
  return tool({
    description:
      "Fetch a URL using a real browser and return its content as a compact accessibility snapshot. " +
      "Handles JavaScript-rendered pages, SPAs, and sites that block simple HTTP requests. " +
