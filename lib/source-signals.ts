...
function scoreSignal(
  ...
  item: Pick<DailySignal, "title" | "url" | "summary">
): number {
...
function extractLinksFromHtml(
  ...
  while (
    ...
    if (!url || !title || title.length < 4 || seen.has(url)) {
      continue;
    }
    if (
      url.startsWith("#") ||
...
function extractLinksFromSitemap(
  ...
  const sitemapIndexEntries = asArray(parsed?.sitemapindex?.sitemap).slice(
    0,
    MAX_SITEMAP_LINKS
  );
...
function resolveLink(
  ...
  if (typeof fallbackId === "string" && fallbackId.startsWith("http")) {
    return fallbackId;
  }
  return fallbackUrl;
}
...
export function normalizeFeedItems(
  ...
  return rankSignals(
    ...
    normalized
      .map((item) => {
        const linkValue = resolveLink(
          item.link,
          item.id ?? item.guid?.["#text"],
          source.url
        );
        const publishedAt =
          item.pubDate ??
...
export async function fetchSignals(
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(
        `[source-signals] ${source.label} (${source.url}) returned ${response.status}`
      );
      return [];
    }
    ...
    if (!payload || payload.length < 20) {
      console.warn(
        `[source-signals] ${source.label} returned an empty or near-empty response`
      );
      return [];
    }
    ...
    if (items.length === 0) {
      console.warn(
        `[source-signals] ${source.label} parsed zero items from ${payload.length} bytes`
      );
    }
    ...
    console.warn(
      `[source-signals] ${source.label} (${source.url}) failed: ${message}`
    );
    return [];
  }
}
