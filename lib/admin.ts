...
export function getAdminEmails(): string[] {
  ...
  return [
    ...new Set(
      configured && configured.length > 0 ? configured : DEFAULT_ADMIN_EMAILS
    ),
  ];
}
...
export function getAdminEmailFromCookieHeader(
  cookieHeader: string | null,
  now = Date.now()
): string | null {
  return getAdminEmailFromSessionToken(
    parseCookieValue(cookieHeader, ADMIN_SESSION_COOKIE),
    now
  );
}
