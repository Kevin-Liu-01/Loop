...
export function peakVolumeHour(
  ...
  return { label: best.label, count: best.total };
}

function splitHalf<T>(items: T[]): { first: T[]; second: T[] } {
  if (items.length === 0) return { first: [], second: [] };
...
export function formatRollingHalfDelta(prev: number, next: number): string | null {
  if (prev === 0 && next === 0) return null;
  if (prev === 0 && next > 0) return "↑ vs prior 12h";
  const pct = Math.round(((next - prev) / prev) * 100);
  if (pct === 0) return "Flat vs prior 12h";
  const sign = pct > 0 ? "+" : "";
...
export function rollingHalfDeltas(series: TimeSeriesBucket[]): {
  ...
  return {
    ...
    interactions: formatRollingHalfDelta(sumInteractions(first), sumInteractions(second)),
...
export function latencyHalfComparison(latency: LatencyBucket[]): string | null {
  ...
  if (a === 0 && b > 0) return "Latency in later 12h";
  if (b === 0 && a > 0) return "Latency in earlier 12h";
