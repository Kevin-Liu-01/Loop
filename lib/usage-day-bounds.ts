...
export function rolling24hWindowEndInclusive(now: Date): {
  start: Date;
  end: Date;
} {
 ...
 */
export function yesterdaySameClockWindow(
  now: Date,
  timeZone: string
): { start: Date; end: Date } {
  ...
  return { end, start: startYesterday };
}

/** Previous contiguous 24h window immediately before the rolling window ending at `now`. */
export function prior24hWindowBeforeRolling(now: Date): {
  start: Date;
  end: Date;
} {
...
export function usageEventsSinceIsoForOverview(
  now: Date,
  timeZone: string
): string {
  const zonedNow = toZonedTime(now, timeZone);
