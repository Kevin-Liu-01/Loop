    ...
    async () => {
      try {
        ...
        if (!skill.creatorClerkUserId) {
          return Response.json(
            { error: "This skill has no creator account linked." },
            { status: 400 }
          );
        }

        const alreadyPurchased = await hasUserPurchasedSkill(
          session.userId,
          slug
        );
        if (alreadyPurchased) {
        ...
        const applicationFee = Math.round(
          skill.price.amount * (feePercent / 100)
        );
...
async function getCreatorConnectAccountId(
  clerkUserId: string
): Promise<string | null> {
  try {
    ...
    return (
      ((user.publicMetadata as Record<string, unknown>)
        ?.stripeConnectAccountId as string) ?? null
    );
  } catch {
    return null;
  }
