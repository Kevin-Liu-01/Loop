...
interface SkillAuthorBadgeProps {
...
export function SkillAuthorBadge({
  ...
  const body = compact ? (
    ...
    >
      ...
      {author?.verified ? <VerifiedTag compact /> : null}
    </span>
  ) : (
    <span
      className={cn(
        ...
        linked &&
          author?.websiteUrl &&
          "hover:border-accent/40 hover:bg-paper-3/90 dark:hover:bg-paper-2/60",
        className
      )}
    >
      <AuthorAvatar
      ...
      />
      <span className="font-medium text-ink">{name}</span>
      {author?.verified ? <VerifiedTag compact={false} /> : null}
    </span>
  );
