...
function statusBadge(op: ActiveOperation): {
...
function ElapsedTimer({ startedAt }: { startedAt: number }) {
  ...
  useEffect(() => {
    const interval = setInterval(
      () => setLabel(computeElapsedLabel(startedAt)),
      1000
    );
...
function OperationRow({ op }: { op: ActiveOperation }) {
  ...
  const stepsLabel =
    op.totalSteps > 0 ? `${op.completedSteps}/${op.totalSteps} sources` : null;
  ...
  return (
    <article
      className={cn(
        ...
        isActive &&
          "border-accent/25 bg-accent/[0.04] shadow-[0_0_0_1px_var(--color-accent)/0.06]",
    ...
    <section aria-label="Active operations" className="grid gap-2">
      {operations.length > 1 && hasCompleted && (
        <div className="flex justify-end">
          <Tip
            content="Remove finished and errored operations from this list"
            side="left"
          >
            <button
