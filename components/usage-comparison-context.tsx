...
import { isUsageComparisonMode } from "@/lib/usage-comparison-modes";
...
export function UsageComparisonProvider({ children }: { children: ReactNode }) {
  const [mode, setModeState] = useState<UsageComparisonMode>(
    "yesterday_same_time"
  );

  useEffect(() => {
    const s = window.localStorage.getItem(STORAGE_KEY);
    if (s && isUsageComparisonMode(s)) {
      setModeState(s);
    }
  }, []);
  ...
  return (
    <UsageComparisonContext.Provider value={value}>
      {children}
    </UsageComparisonContext.Provider>
  );
}
