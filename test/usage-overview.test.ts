...
test("computeUsageTotals splits kinds", () => {
  const events = [
    ...
    ev("2024-06-11T10:01:00.000Z", "api_call", { durationMs: 10, ok: true, status: 200 }),
...
test("buildUsageOverview windows: today vs yesterday vs rolling", () => {
  ...
  assert.equal(o.totalsRolling24h.pageViews, 4, "rolling from 10 Jun 15:00 through now");
  assert.equal(o.vsYesterday.pageViews, "+100% vs y'day same time");
  assert.equal(o.comparisons.yesterday_same_time.pageViews, "+100% vs y'day same time");

  const sumRolling = o.timeSeries.reduce((s, b) => s + b.total, 0);
  assert.equal(sumRolling, o.totalsRolling24h.pageViews + o.totalsRolling24h.interactions + o.totalsRolling24h.apiCalls);
});
