...
test("CATEGORY_REGISTRY has entries to seed", () => {
  assert.ok(
    CATEGORY_REGISTRY.length > 0,
    "Category registry should not be empty"
  );
});
...
test("Each category has required fields for migration", () => {
  for (const category of CATEGORY_REGISTRY) {
    ...
    assert.ok(
      category.strapline !== undefined,
      `Category "${category.slug}" missing strapline`
    );
    assert.ok(
      category.description !== undefined,
      `Category "${category.slug}" missing description`
    );
    assert.ok(
      Array.isArray(category.keywords),
      `Category "${category.slug}" keywords should be an array`
    );
    assert.ok(
      Array.isArray(category.sources),
      `Category "${category.slug}" sources should be an array`
    );
  }
});
  ...
  for (const category of CATEGORY_REGISTRY) {
    for (const source of category.sources) {
      ...
      assert.ok(
        ...
        `Source in "${category.slug}" tags should be an array`
      );
    }
  }
});
...
test("Migration file references for JSON stores exist as constants", () => {
  ...
  for (const file of expectedFiles) {
    assert.ok(
      ...
      `Expected local JSON file pattern: ${file}`
    );
