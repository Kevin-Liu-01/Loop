...
test("disabled automation is never due", () => {
  ...
  assert.equal(
    isUserSkillAutomationDue(disabled, new Date("2026-03-30T12:00:00Z")),
    false
  );
});
...
test("paused automation is never due", () => {
  ...
  assert.equal(
    isUserSkillAutomationDue(paused, new Date("2026-03-30T12:00:00Z")),
    false
  );
});
...
test("skill with no sources is never due", () => {
  ...
  assert.equal(
    isUserSkillAutomationDue(skill, new Date("2026-03-30T12:00:00Z")),
    false
  );
});
...
test("weekly skill does not fire on Monday if less than 7 days elapsed", () => {
  const skill = makeActiveSkill({
    cadence: "weekly",
    lastRunAt: "2026-03-27T12:00:00Z",
  });
...
test("consecutiveFailures of 0 or undefined behaves normally", () => {
  const skill0 = makeActiveSkill({
    consecutiveFailures: 0,
    lastRunAt: undefined,
  });
  assert.equal(
    isUserSkillAutomationDue(skill0, new Date("2026-03-30T12:00:00Z")),
    true
  );

  const skillUndef = makeActiveSkill({ lastRunAt: undefined });
  assert.equal(
    isUserSkillAutomationDue(skillUndef, new Date("2026-03-30T12:00:00Z")),
    true
  );
});
...
test("invalid lastRunAt date is treated as never run", () => {
  ...
  assert.equal(
    isUserSkillAutomationDue(withBadDate, new Date("2026-03-30T12:00:00Z")),
    true
  );
});
