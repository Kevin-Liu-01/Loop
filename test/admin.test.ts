...
test("normalizeAdminEmail trims and lowercases", () => {
  assert.equal(
    normalizeAdminEmail("  KK23907751@GMAIL.COM "),
...
test("admin session tokens round-trip the allowlisted operator email", () => {
  ...
  assert.equal(
    getAdminEmailFromSessionToken(token, now + 1000),
    "kk23907751@gmail.com"
  );
});
...
test("expired or tampered admin session tokens are rejected", () => {
  ...
  assert.equal(
    readAdminSessionToken(token, now + 1000 * 60 * 60 * 24 * 31),
    null
  );
...
test("admin email can be read from the cookie header", () => {
  ...
  assert.equal(
    getAdminEmailFromCookieHeader(cookieHeader, now + 1000),
    "kk23907751@gmail.com"
  );
});
